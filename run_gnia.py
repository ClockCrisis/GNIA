...
#又是一个，手动加载参数
    ...
    ori_model.load_state_dict(torch.load('./processed_data/model_ori_rgcn.pth', map_location=device))
    ...
    num_decoder_model.load_state_dict(torch.load('./processed_data/model_num_decoder.pth'))
    cat_decoder_model = cat_decoder().to(device)
    cat_decoder_model.load_state_dict(torch.load('./processed_data/model_cat_decoder.pth'))
    ...
    # torch.save(RGCN_dict, './processed_data/model_RGCN.pth')
...
# todo 不分解w
    ...
    torch.save(RGCN_weight_model.state_dict(), './processed_data/model_RGCN_weight.pth')
        ...
        for batch_x,_ in batch_loader:
            ...
            for train_batch in batch_x:
                try:
                    ...
                    ori = labels_np[target].item()  #攻击之前的label
                    best_wrong_label = 0  #攻击之后期望的label
