...
sys.path.append('..')
from utils import *
from model import oriRGCN, num_decoder, cat_decoder, BotGCN, encoder, num_encoder, RGCN_weight,GAT_cat_decoder,GAT_num_decoder
...
def main(opts):
    ...
    # print("using gat_num_decoder")
    # num_decoder_model = GAT_num_decoder().to(device)
    # num_decoder_model.load_state_dict(torch.load('./processed_data/model_gat_num_decoder.pth', map_location='cpu'))
    ...
    #GAT的
    # print("using gat_cat_decoder")
    # cat_decoder_model = GAT_cat_decoder().to(device)
    # cat_decoder_model.load_state_dict(torch.load('./processed_data/model_gat_cat_decoder.pth', map_location='cpu'))
    ...
    detect_model = BotGCN().to(device)
    detect_model.load_state_dict(torch.load('./processed_data/model_GCN.pth', map_location='cpu'))
    ...
    #节点嵌入
    node_emb = detect_model(des_tensor, tweets_tensor, num_prop, category_prop, edge_index)
    ...
    logits = detect_model(des_tensor, tweets_tensor, num_prop, category_prop, edge_index)
    ...
    for dset in ['test']:
        ...
        for batch in names[dset + '_mask_bot']:
            target = np.array([batch])#这个就是目标节点
            ...
            #随机选择生成的边，边嵌入生成消融实验
            # masked_score_idx = masked_score_idx[torch.randint(0, len(masked_score_idx), (1,))]

            #直接将目标属性的feat赋值给注入节点,节点嵌入消融实验
            # print(feat.shape)
            # print(target.shape)
            # print(target)
            # inj_feat =feat[target.squeeze(0)]#麻了，这行一加上直接90多变0了，也不用波动这么大吧
            # print(inj_feat)
            ...
            # print(new_number)
            #注意力的
            # new_num = num_decoder_model(inj_feat[16:24].reshape(1,8))
            # new_number = (new_num * num_std + num_mean).abs().round().reshape(5)
            # print(new_number)
            ...
            #GAT
            # print(inj_feat)
            # print("inj_feat shape:", inj_feat.shape)  # 应该是 [num_nodes, num_features]
            # print("inj_feat[24:] shape:", inj_feat[24:].shape)  # 检查切片后的形状
            # print("edge_index shape:", edge_index.shape)
            # new_cat_float = cat_decoder_model(np.tile(inj_feat[24:], (5301, 1)),edge_index)[1,1]
            # # feat_cpu = inj_feat[24:].detach().cpu().numpy()  # 从 CUDA 移到 CPU 并转成 numpy 数组
            # # tiled_feat = np.tile(feat_cpu, (5301, 1))  # 使用 numpy 进行扩展
            # # new_cat_float = cat_decoder_model(tiled_feat, edge_index)[1, 1]
            ...
            new_tweets_tensor = torch.cat((tweets_tensor, torch.zeros(1, 768).to(device)), 0)#推文置零
            new_num_prop = torch.cat((num_prop, new_num.unsqueeze(0)), 0)#数字属性
            new_category_prop = torch.cat((category_prop, new_cat), 0)#分类属性
