...
class GNIA(nn.Module):
    def __init__(self, labels, feat_dim, weight, discrete, device, tar_num=1, feat_max=None, feat_min=None, feat_num=None, attr_tau=None, edge_tau=None):
        super(GNIA,self).__init__()
        self.labels = labels
#         self.budget = budget
        self.feat_dim = feat_dim
        self.feat_num = feat_num
        self.add_node_agent = AttrGeneration(self.labels, attr_tau, self.feat_dim, weight, discrete, device, tar_num, feat_max, feat_min).to(device)
