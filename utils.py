...
# -------------------------------------- After Attack ----------------------------------
def gen_extend_edge_index(edge_index, n, edges, sub_graph_nodes, device):
    # sparse tensor

    edge_idx = sub_graph_nodes.to(device)
    sub_mask_shape = edge_idx.shape[1]
    res = n * torch.ones(sub_mask_shape, device=device).unsqueeze(0).long()
    # print("edge_idx device:", edge_idx.device)
    # print("res device:", res.device)
    extend_i0 = torch.cat((res, edge_idx), 0).to(device)
    # print(999)
