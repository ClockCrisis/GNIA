...
sys.path.append('..')
from utils import *
from model import oriRGCN, num_decoder, cat_decoder, BotRGCN, encoder, num_encoder, RGCN_weight,HGTDetector
...
def main(opts):
    ...
    detect_model = HGTDetector().to(device)
    detect_model.load_state_dict(torch.load('./processed_data/model_HGT.pth', map_location='cpu'))
    ...
    for dset in ['test']:
        ...
        for batch in names[dset + '_mask_bot']:
            ...
            new_logits = detect_model(new_des_tensor, new_tweets_tensor, new_num_prop, new_category_prop,
                                   new_edge_index, new_edge_type)  # (10005,41)
